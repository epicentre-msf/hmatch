order_within <- function(x, col, col_split) {
  ...
  x_split_order <- lapply(x_split, function(d) {
    d[order(d[[col]]), , drop = FALSE]
  })
...
tokenize_cols <- function(x, by, split, prefix = "token_") {
  ...
  for (j in by) {
    bind_[[j]] <- tokenize(bind_[[j]], split = split)
  }
...
rbind_dfs <- function(x, y) {
  ...
  for (j in cols_add_to_x) {
    x <- add_column(x, j, NA)
  }
  for (j in cols_add_to_y) {
    y <- add_column(y, j, NA)
  }
