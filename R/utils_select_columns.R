...
select_pattern <- function(x, pattern) {
  ...
  if (length(by) == 0L) {
    stop("No column names were matched by argument ", backtick(var), call. = FALSE)
  }
...
select_by <- function(x, by) {
  ...
  if (length(by_no_match) > 0) {
    stop(
      "The following element(s) of argument ",
      backtick(var),
      " could not be matched to a column name: ",
      vec_paste_c(by_no_match),
      call. = FALSE
    )
  }
  ...
  if (!all(by == by_match)) {
    warning(
      "The order of columns within ",
      backtick(var),
      " differs from the ",
      "order matched: ",
      vec_paste_c(by),
      " vs. ",
      vec_paste_c(by_match),
      call. = FALSE
    )
  }
